<?php include('components/head.in.php') ?>
<?php include('components/navbar-head.in.php') ?>
<div>
<section #aboutus class="heading-section img-fluid" style="
  background-image: linear-gradient(rgb(14 10 64 / 63%), rgba(14, 13, 12, 0.7)),
  url('./assets/elvservices3.jpg');">
    <div id="aboutUsContentID" class="caption" style="text-align: center;">
        <div class="row " style="width: 100%;">
            <div class="col-md-12 " data-aos="slide-down">
                <h3 style="font-weight: 700">SERVICES</h3>
            </div>
        </div>
    </div>
</section>
<section class="service-details service-top-align mb-1">
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320">
    <path fill="#fff" fill-opacity="1" d="M0,288L80,272C160,256,320,224,480,224C640,224,800,256,960,256C1120,256,1280,224,1360,208L1440,192L1440,0L1360,0C1280,0,1120,0,960,0C800,0,640,0,480,0C320,0,160,0,80,0L0,0Z">
</path>
</svg>
        <div class="conatiner" >
            <div class="conatiner">
                <div class="row text-center align-items-center justify-content-center" style="width: 100%;">
                    <div class="col-md-6 text-heading-new mb-5" data-aos="slide-up">
                        <h1>One stop for comprehensive Consultancy</h1>
                        <!-- <p class="service-subheading">We are a full services Electrical Contractor</p> -->
                    </div>
                </div>
            </div>

            <div class="container mb-5">
                <div class="row mb-5 " style="width: 100%;">
                    <div class="col-md-4 mb-5" data-aos="slide-right">
                        <div class="card" style="width: 18rem;">
                            <img src="./assets/elvsoln.jpg" class="card-img-top" alt="...">
                            <div class="card-body">
                                <h5 class="card-title"><b>Digital Consultancy </b></h5>
                                <p class="card-text">We deliver the complete range of consulting services</p>
                                <!-- <a href="#" class="btn btn-primary">Go somewhere</a> -->
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4 mb-5" data-aos="slide-left">
                        <div class="card" style="width: 18rem;">
                            <img src="./assets/ictbanner5.png" class="card-img-top" alt="..." height="115px">
                            <div class="card-body">
                                <h5 class="card-title"><b>Digital Transformation</b></h5>
                                <p class="card-text">We provide all kinds of solutions</p>
                                <!-- <a href="#" class="btn btn-primary">Go somewhere</a> -->
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4" data-aos="slide-left">
                        <div class="card" style="width: 18rem;">
                            <img src="./assets/elvservice1.jpg" class="card-img-top" alt="..." height="115px">
                            <div class="card-body">
                                <h5 class="card-title"><b>Digital Engagement</b></h5>
                                <p class="card-text">We provide all kinds of developing and maintenance services</p>
                                <!-- <a href="#" class="btn btn-primary">Go somewhere</a> -->
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>

    </section>
</div>   

    <?php include('components/footer.in.php') ?>